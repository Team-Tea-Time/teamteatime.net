...
There are 4 configuration files in this version; **api**, **frontend**, **general**, and **integration**. They should be published to `app/config/forum.{config key}.php` after running `php artisan vendor:publish` as part of the installation.
...
* `enable`: Whether or not to enable the frontend routes.
* `preset`: The UI preset to use. See [UI Presets](/docs/laravel-forum/6.x/front-end/ui-presets).
...
<Alert type="info">
    `display_trashed_posts` is used in conjunction with [Policies](/docs/laravel-forum/5/policies/) as follows:
