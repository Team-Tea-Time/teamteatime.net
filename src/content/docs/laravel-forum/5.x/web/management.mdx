...
<Alert type="info">
    **In 5.1.0 onwards**, the **Private** property also determines access to all of the category's threads and child categories, even those set as public. A user cannot access a category if it has an ancestor with this property set to `true` and a `CategoryPolicy::view` ability check fails on that ancestor for the user.
</Alert>
