...
<Alert type="warning">
    The `blade-bootstrap` preset is deprecated as of package version **7.0.3**. It is recommended to use the `blade-tailwind` preset instead.
</Alert>
...
<Alert type="info">
    **New as of 7.0.3**
    ...
    Presets can be marked as unsupported by returning `false` from the `isSupported` method. The `forum:preset-list` and `forum:preset-install` commands will mark an unsupported preset as such and ask for confirmation before installing it. You may still use unsupported presets, but they may not function correctly.
</Alert>
